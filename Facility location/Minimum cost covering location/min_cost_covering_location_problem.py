...
def buildModel(data: dict) -> pyomo.ConcreteModel():
    ...
    model.obj = pyomo.Objective(expr=sum(model.f[i]*model.y[i] for i in model.facilityRange), sense=pyomo.minimize)
    # Add covering constraints
    model.coveringCsts = pyomo.ConstraintList()
    for j in model.customerRange:
        model.coveringCsts.add(expr=sum(model.a[i][j]*model.y[i] for i in model.facilityRange) >= model.b[j])
